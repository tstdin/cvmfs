...
string SignatureManager::GenerateKeyText(RSA *pubkey) {
  if (!pubkey)
    return "";
  ...
  if (bp == NULL) {
    LogCvmfs(kLogSignature, kLogDebug | kLogSyslogErr,
            "Failed to allocate memory for pubkey");
    return "";
  }
  if (!PEM_write_bio_RSA_PUBKEY(bp, pubkey)) {
    LogCvmfs(kLogSignature, kLogDebug | kLogSyslogErr,
             "Failed to write pubkey to memory");
    return "";
  }
  char *bio_pubkey_text;
  long bytes = BIO_get_mem_data(bp, &bio_pubkey_text);

  string result(bio_pubkey_text, bytes);
  BIO_free(bp);
  return result;
}
                                 ...
                                 const unsigned signature_size)
{
  for (unsigned i = 0, s = public_keys_.size(); i < s; ++i) {
    ...
    {
      free(to);
      last_successful_pubkey_ = GenerateKeyText(public_keys_[i]);
      return true;
    }
