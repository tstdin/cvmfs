  ...
  const bool stop_for_tweaks)
{
  LogCvmfs(kLogCvmfs, kLogStdout,
           "CernVM-FS workaround: serialized processing of file catalogs!");
  ...
  for (; i != iend; ++i) {
    LogCvmfs(kLogCatalog, kLogStdout, "process catalog '%s'",
             (*i)->path().c_str());
    FinalizeCatalog(*i, stop_for_tweaks);
    ...
    if (!zlib::CompressPath2Path((*i)->database_path(),
                                 ...
                                 &hash_catalog))
    {
      PrintError("could not compress catalog " + (*i)->path().ToString());
      assert(false);
    }
    ...
    if ((*i)->HasParent()) {
      ...
      parent->UpdateNestedCatalog((*i)->path().ToString(), hash_catalog,
